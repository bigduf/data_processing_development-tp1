...
object Main {
  def main(args: Array[String]) {
    ...
    val sqlQuery = "SELECT * FROM news WHERE containsWordGlobalWarming = true"
    ...
    enrichedDataset.select("*").where("containsWordGlobalWarming = true")
    ...
    enrichedDataset.write
      ...
      .partitionBy("date", "media").parquet("output.parquet")
  }
}